...
local function OnEvent(self, event, ...)
	self.mSettings = {
		Name = mTextName,
		IconTexture = E.db[mPlugin].mDock.guild.path,
		Notifications = true,
		Text = true,
		Spezial = false,
	}
	...
	if E.db[mPlugin].mDock.guild.color == "custom" then
		local r, g, b = E.db[mPlugin].mDock.fontcolor.r, E.db[mPlugin].mDock.fontcolor.g, E.db[mPlugin].mDock.fontcolor.b
	...
	if MediaTagGameVersion.retail then
		mMT:ShowHideNotification(self, (GuildMicroButtonMixin:HasUnseenInvitations() or CommunitiesUtil.DoesAnyCommunityHaveUnreadMessages()))
	end
...
DT:RegisterDatatext(mTextName, "mDock", {'CHAT_MSG_SYSTEM', 'GUILD_ROSTER_UPDATE', 'PLAYER_GUILD_UPDATE', 'GUILD_MOTD', 'MODIFIER_STATE_CHANGED'}, OnEvent, nil, OnClick, OnEnter, OnLeave, mText, nil, nil)