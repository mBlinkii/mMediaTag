...
local mText = format("mMediaTag %s", L["Soul Ash"])
local mTextName = "mSoulAsh"
local mCurrencyID = 1828
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if E.db[mPlugin].mSoulAsh.hide and CurrencValue == 0 then
			hideCurrency = true
		else
			hideCurrency = false
		end
		...
		if not hideCurrency then
			if E.db[mPlugin].mSoulAsh.name then
			...
			if E.db[mPlugin].mSoulAsh.icon then
				if TextJustify == "RIGHT" then
			...
			if E.db[mPlugin].mSoulAsh.short then
				CurrencValue = E:ShortValue(info.quantity, 2)
				maxValue = E:ShortValue(info.maxQuantity, 2)
			end

			if E.db[mPlugin].mSoulAsh.showmax then
				CurrencValue = CurrencValue .. " / " .. maxValue
			end

			local CurrencyTextSring = displayString

			if E.db[mPlugin].mSoulAsh.style == "color" then
				if TextJustify == "RIGHT" then
					CurrencyTextSring = "|CFFF39C12%s|r%s"
				else
					CurrencyTextSring = "%s|CFFF39C12%s|r"
				end
			elseif E.db[mPlugin].mSoulAsh.style == "white" then
