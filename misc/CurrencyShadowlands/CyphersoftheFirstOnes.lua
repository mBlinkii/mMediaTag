...
local mText = format("mMediaTag %s", L["Cyphers of the First Ones"])
local mTextName = "mCyphersFirstOnes"
local mCurrencyID = 1979
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if E.db[mPlugin].mCyphersFirstOnes.hide and CurrencValue == 0 then
			hideCurrency = true
		else
			hideCurrency = false
		end
		...
		if not hideCurrency then
			if E.db[mPlugin].mCyphersFirstOnes.name then
			...
			if E.db[mPlugin].mCyphersFirstOnes.icon then
				if TextJustify == "RIGHT" then
			...
			if E.db[mPlugin].mCyphersFirstOnes.short then
				CurrencValue = E:ShortValue(info.quantity, 2)
			end

			local CurrencyTextSring = displayString

			if E.db[mPlugin].mCyphersFirstOnes.style == "color" then
				if TextJustify == "RIGHT" then
					CurrencyTextSring = "|CFFAED0E1%s|r%s"
				else
					CurrencyTextSring = "%s|CFFAED0E1%s|r"
				end
			elseif E.db[mPlugin].mCyphersFirstOnes.style == "white" then
