...
local mText = format("mMediaTag %s", L["Grateful Offering"])
local mTextName = "mGratefulOffering"
local mCurrencyID = 1885
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if E.db[mPlugin].mGratefulOffering.hide and CurrencValue == 0 then
			hideCurrency = true
		else
			hideCurrency = false
		end
		...
		if not hideCurrency then
			if E.db[mPlugin].mGratefulOffering.name then
			...
			if E.db[mPlugin].mGratefulOffering.icon then
				if TextJustify == "RIGHT" then
			...
			if E.db[mPlugin].mGratefulOffering.short then
				CurrencValue = E:ShortValue(info.quantity, 2)
			end

			local CurrencyTextSring = displayString

			if E.db[mPlugin].mGratefulOffering.style == "color" then
				if TextJustify == "RIGHT" then
					CurrencyTextSring = "|CFFF4D03F%s|r%s"
				else
					CurrencyTextSring = "%s|CFFF4D03F%s|r"
				end
			elseif E.db[mPlugin].mGratefulOffering.style == "white" then
