...
local mText = format("mMediaTag %s", L["Tower Knowledge"])
local mTextName = "mTowerKnowledge"
local mCurrencyID = 1904
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if E.db[mPlugin].mTowerKnowledge.hide and CurrencValue == 0 then
			hideCurrency = true
		else
			hideCurrency = false
		end
		...
		if not hideCurrency then
			if E.db[mPlugin].mTowerKnowledge.name then
			...
			if E.db[mPlugin].mTowerKnowledge.icon then
				if TextJustify == "RIGHT" then
			...
			if E.db[mPlugin].mTowerKnowledge.short then
				CurrencValue = E:ShortValue(info.quantity, 2)
				maxValue = E:ShortValue(info.maxQuantity, 2)
			end

			if E.db[mPlugin].mTowerKnowledge.showmax then
				CurrencValue = CurrencValue .. " / " .. maxValue
			end

			local CurrencyTextSring = displayString

			if E.db[mPlugin].mTowerKnowledge.style == "color" then
				if TextJustify == "RIGHT" then
					CurrencyTextSring = "|CFFCCF7E4%s|r%s"
				else
					CurrencyTextSring = "%s|CFFCCF7E4%s|r"
				end
			elseif E.db[mPlugin].mTowerKnowledge.style == "white" then
