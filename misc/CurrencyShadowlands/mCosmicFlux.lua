...
local mText = format("mMediaTag %s", L["Cosmic Flux"])
local mTextName = "mCosmicFlux"
local mCurrencyID = 2009
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if E.db[mPlugin].mCosmicFlux.hide and CurrencValue == 0 then
			hideCurrency = true
		else
			hideCurrency = false
		end
		...
		if not hideCurrency then
			if E.db[mPlugin].mCosmicFlux.name then
			...
			if E.db[mPlugin].mCosmicFlux.icon then
				if TextJustify == "RIGHT" then
			...
			if E.db[mPlugin].mCosmicFlux.short then
				CurrencValue = E:ShortValue(info.quantity, 2)
			end

			local CurrencyTextSring = displayString

			if E.db[mPlugin].mCosmicFlux.style == "color" then
				if TextJustify == "RIGHT" then
					CurrencyTextSring = "|CFFFFD968%s|r%s"
				else
					CurrencyTextSring = "%s|CFFFFD968%s|r"
				end
			elseif E.db[mPlugin].mCosmicFlux.style == "white" then
