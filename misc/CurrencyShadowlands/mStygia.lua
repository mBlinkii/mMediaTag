...
local mText = format("mMediaTag %s", L["Stygia"])
local mTextName = "mStygia"
local mCurrencyID = 1767
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if E.db[mPlugin].mStygia.hide and CurrencValue == 0 then
			hideCurrency = true
		else
			hideCurrency = false
		end
		...
		if not hideCurrency then
			if E.db[mPlugin].mStygia.name then
			...
			if E.db[mPlugin].mStygia.icon then
				if TextJustify == "RIGHT" then
			...
			if E.db[mPlugin].mStygia.short then
				CurrencValue = E:ShortValue(info.quantity, 2)
			end

			local CurrencyTextSring = displayString

			if E.db[mPlugin].mStygia.style == "color" then
				if TextJustify == "RIGHT" then
					CurrencyTextSring = "|CFF85C1E9%s|r%s"
				else
					CurrencyTextSring = "%s|CFF85C1E9%s|r"
				end
			elseif E.db[mPlugin].mStygia.style == "white" then
