...
local mText = format("mMediaTag %s", L["Soul Cinders"])
local mTextName = "mSoulCinders"
local mCurrencyID = 1906
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if not hideCurrency then
			...
			if E.db[mPlugin].mSoulCinders.style == "color" then
				if TextJustify == "RIGHT" then
					CurrencyTextSring = "|CFFFF5500%s|r%s"
				else
					CurrencyTextSring = "%s|CFFFF5500%s|r"
				end
