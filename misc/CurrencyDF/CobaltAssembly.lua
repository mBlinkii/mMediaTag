...
local mTextName = "mCobaltAssembly"
local mCurrencyID = 2134
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if E.db[mPlugin].mCobaltAssembly.hide and CurrencValue == 0 then
			hideCurrency = true
		else
			hideCurrency = false
		end
		...
		if not hideCurrency then
			if E.db[mPlugin].mCobaltAssembly.name then
			...
			if E.db[mPlugin].mCobaltAssembly.icon then
				if TextJustify == "RIGHT" then
