...
local mTextName = "mDragonGlyphEmbers"
local mCurrencyID = 2045
...
local function OnEvent(self, event, ...)
	...
	if info then
		...
		if E.db[mPlugin].mDragonGlyphEmbers.hide and CurrencValue == 0 then
			hideCurrency = true
		else
			hideCurrency = false
		end
		...
		if not hideCurrency then
			if E.db[mPlugin].mDragonGlyphEmbers.name then
			...
			if E.db[mPlugin].mDragonGlyphEmbers.icon then
				if TextJustify == "RIGHT" then
			...
			if E.db[mPlugin].mDragonGlyphEmbers.short then
				CurrencValue = E:ShortValue(info.quantity, 2)
			end

			local CurrencyTextSring = displayString

			if E.db[mPlugin].mDragonGlyphEmbers.style == "color" then
				if TextJustify == "RIGHT" then
					CurrencyTextSring = "|CFF0873B9%s|r%s"
				else
					CurrencyTextSring = "%s|CFF0873B9%s|r"
				end
			elseif E.db[mPlugin].mDragonGlyphEmbers.style == "white" then
