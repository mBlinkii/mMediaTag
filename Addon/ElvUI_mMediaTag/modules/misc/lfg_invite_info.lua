...
function module:Demo(show)
	if show then
		module.info_screen.lable:SetText(format("%s \n%s \n%s", mMT:TC(L["GROUP NAME"] .. " - " .. L["ACTIVITY NAME"], "line_a"), mMT:TC(L["ACTIVITY"], "line_b"), mMT:TC(L["DIFFICULTY"], "line_c")))
...
function module:Initialize(demo)
	if E.db.mMT.lfg_invite_info.enable then
		...
		module.info_screen.lable:SetFont(LSM:Fetch("font", module.db.font.font), module.db.font.size, module.db.font.fontflag)
		...
		if module.db.icon ~= "none" then
			...
			module.info_screen.icon:SetHeight(module.db.font.size * 4)
			module.info_screen.icon:SetWidth(module.db.font.size * 4)
...
function module:LFG_LIST_JOINED_GROUP(_, searchResultID, groupName)
	...
	if id then
		...
		module.info_screen.lable:SetText(format("%s \n%s \n%s", mMT:TC(groupName .. " - " .. name, "line_a"), mMT:TC(activity, "line_b"), mMT:TC(difficulty, "line_c")))
