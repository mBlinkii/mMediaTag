...
local function EmbedDetailsInstances(embedded, windows)
    ...
    for _, instance in Details:ListInstances() do
        ...
        if instance:IsEnabled() then
            ...
            local bottomOfs = (instance.show_statusbar and 14 or 0)
                            + (instance.toolbar_side == 2 and 20 or 0)
...
local function DetailsEmbedded()
    ...
    if not module.detailsEmbedded then
        ...
        if module.toggle then
            module.detailsToggle = module.detailsToggle
                or CreateToggleButton(
                    backdrop, embedded,
                    { bbr, bbg, bbb, bba },
                    { bdr, bdg, bdb, bda },
                    chatHeight, isRightChat
                )
        end
        ...
        for i = 1, chat:GetNumPoints() do
            local point, _, relPoint, x, y = chat:GetPoint(i)
            embedded:SetPoint(point, chat, relPoint, x, y)
        end

        if chat.tex then
            local texPath = isRightChat
                and E.db.chat.panelBackdropNameRight
