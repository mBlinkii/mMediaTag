...
local customIcons = {
	colored = "Interface\\AddOns\\ElvUI_mMediaTag\\media\\icons\\datatexts\\secondary_a.tga",
	white = "Interface\\AddOns\\ElvUI_mMediaTag\\media\\icons\\datatexts\\secondary_b.tga",
}
...
local function OnEnter(self)
	profession = select(2, GetProfessions())
	if profession then
		...
		DT.tooltip:AddDoubleLine((icon and (icon .. " ") or "") .. mMT:SetTextColor(name), skillModifier ~= 0 and UpdateSkillString(skillLevel, maxSkillLevel, skillModifier) or "")
	else
...
DT:RegisterDatatext( "mMT - Secondary Professions", mMT.Name, { "TRADE_SKILL_DETAILS_UPDATE", "SKILL_LINES_CHANGED" }, OnEvent, nil, OnClick, OnEnter, OnLeave, L["Secondary Professions"], nil, ValueColorUpdate )
