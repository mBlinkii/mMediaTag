...
local function addNotPlayed(ScoreTable, mapID, affix)
    if not ScoreTable[mapID][affix] then
...
local function GetDungeonScores()
    ...
    if ScoreTable and map_table then
        ...
        for i = 1, #map_table do
            ...
            -- Ensure ScoreTable[mapID] is initialized
            if not ScoreTable[mapID] then
                ScoreTable[mapID] = {}
            end

            -- Ensure affixes are initialized
            if not ScoreTable[mapID][tyrannical] then
                addNotPlayed(ScoreTable, mapID, tyrannical)
            end
            if not ScoreTable[mapID][fortified] then
                addNotPlayed(ScoreTable, mapID, fortified)
            end

            local nameString = format("%s %s", mMT:mIcon(ScoreTable[mapID].icon), ScoreTable[mapID].name)
            local scoreString = format("|C%s%s|r | |C%s%s|r | |C%s%s|r",
                ScoreTable[mapID][tyrannical].color, ScoreTable[mapID][tyrannical].level,
                ScoreTable[mapID][fortified].color, ScoreTable[mapID][fortified].level,
                ScoreTable[mapID].color, ScoreTable[mapID].score)
