...
function mMT:mDropDown(list, frame, menuparent, ButtonWidth, HideDelay)
	...
	for i = 1, #list do
		...
		if list[i].text then
			...
			frame.buttons[i].text:FontTemplate(font, fontSize, fontFlag)
		...
		if list[i].SecondText then
			...
			frame.buttons[i].SecondText:FontTemplate(font, fontSize, fontFlag)
			frame.buttons[i].SecondText:SetJustifyH("RIGHT")
		end

		if list[i].tooltip then frame.buttons[i].tooltip = list[i].tooltip end
		...
		if list[i].text and frame.buttons[i].text then
			if list[i].color then list[i].text = format("%s%s|r", list[i].color, list[i].text) end
		...
		if list[i].SecondText and frame.buttons[i].SecondText then frame.buttons[i].SecondText:SetText(list[i].SecondText or "") end
		...
		if frame.buttons[i].text then
			...
			if height ~= 0 then BUTTON_HEIGHT = (height > BUTTON_HEIGHT) and height or BUTTON_HEIGHT end

			if width ~= 0 then
				if frame.buttons[i].SecondText then
					local secondWidth = frame.buttons[i].SecondText:GetStringWidth()

					if secondWidth ~= 0 then width = width + secondWidth end
				end
